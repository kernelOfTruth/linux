...
void print_scheduler_version(void)
{
	...
	printk(KERN_INFO "BFS enhancement patchset v4.3_0466_4_vrq3 by Alfred Chen.\n");
}
...
/*
 * calculate scost(switch cost) based on the next task switching-in
 *
 * minimal scost is 1.
 * If it's cache cool, 4*scost.
 * If it's not a kernel thread, 2*scost.
 */
static inline void
...
{\
	int cpu = cpu_of(rq);\
\
	if (is_task_deactivate_sched(rq, cpu, prev, pswitch_count))\
...
static struct task_struct *\
need_other_cpu_choose_task##subfix(struct rq *rq,\
				   struct task_struct *prev,\
				   unsigned long **pswitch_count)\
{\
	int cpu = cpu_of(rq);\
...
static void __sched __schedule(void)
{
	...
	next = rq->choose_task_func(rq, prev, &switch_count);
